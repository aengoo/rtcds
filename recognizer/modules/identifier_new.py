...
def box_adapt(boxes: np.ndarray, res: tuple, rat=1.):
    res_boxes = boxes[:, :4] * np.array(res[:2] + res[:2])
    ...
    pad_res_boxes = np.concatenate((boxes_center - pad_boxes_rad, boxes_center + pad_boxes_rad, boxes[:, 4:]), axis=1)
    return pad_res_boxes


class Identifier:
    def __init__(self, face_path: str, idt_res: str, box_ratio: float, timer: Timer = None,
                 is_eval=False, landms68: bool = False):

        self.encoder = face_encoder.EncodeFace(face_path, landms68)
        self.faces = []
    ...
    def run(self, img_idt, boxes):
        ...
        for tbox in boxes:
            ...
            face_name, face_dist, face_std_score = self.encoder.match_face(cropped)
